...
struct PulseInfo
{
 bool DBusEnable; /* Set if the DBus is used instead of Pulse */
 bool Enable;  /* Should a pulse generator be used */
...
static bool bErCardListInitDone = epicsFalse;
...
|*
\**************************************************************************************************/
void ErSetDg(ErCardStruct *pCard, int Channel, bool Enable, 
			epicsUInt32 Delay, epicsUInt32 Width, 
			epicsUInt16 Prescaler, bool Pol)
{
	struct MrfErRegs *pEr = (struct MrfErRegs *)pCard->pEr;
	struct LinuxErCardStruct *pLinuxErCard = ercard_to_linuxercard(pCard);
